...
DLL_EXPORT params_t crc_params(uint8_t width, uint64_t poly, uint64_t init, bool refin, bool refout, uint64_t xorout, uint64_t check, uint8_t *error);

/* List of crc_params errors */
DLL_EXPORT enum crc_params_errors {
...
DLL_EXPORT void crc_print_error(uint8_t error);

/* Calculate the CRC using the table-based algorithm.
   Use params.init as the initial CRC value.*/
DLL_EXPORT uint64_t crc_table(params_t *params, uint64_t crc, unsigned char const *buf, uint64_t len);
...
/* Calculate the CRC using the SIMD algorithm. Falls back to the table-based
   ...
   initial CRC value.*/
DLL_EXPORT uint64_t crc_calc(params_t *params, uint64_t crc, unsigned char const *buf, uint64_t len);
...
DLL_EXPORT uint64_t crc_combine_constant(params_t *params, uint64_t len);
...
/* Combine two CRCs. xp is the constant precomputed using crc_combine_constant.
   ...
   same for all CRCs. */
DLL_EXPORT uint64_t crc_combine_fixed(params_t *params, uint64_t crc, uint64_t crc2, uint64_t xp);

/* Combine two CRCs. len is the length of the second CRC's message. */
DLL_EXPORT uint64_t crc_combine(params_t *params, uint64_t crc, uint64_t crc2, uint64_t len);
